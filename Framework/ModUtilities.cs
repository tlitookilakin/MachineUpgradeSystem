...
namespace MachineUpgradeSystem.Framework
{
	internal static class ModUtilities
	{
		...
		public static Expression GetValue(this Expression source, Type t, string name, out Type type)
		{
			if (t.GetProperty(name, BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic) is PropertyInfo p)
			{
				type = p.PropertyType;
				return Expression.Call(source, p.SetMethod!);
			}

			if (t.GetField(name, BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic) is FieldInfo f)
			{
				type = f.FieldType;
				return Expression.Field(source, f);
			}

			throw new InvalidOperationException();
		}

		public static bool TryFindAssembly(string name, [NotNullWhen(true)] out Assembly? asm)
		{
			asm = AppDomain.CurrentDomain.GetAssemblies().FirstOrDefault(a => a.GetName().Name.EqualsIgnoreCase(name));
		...
		public static int GetLocalIndex(this CodeInstruction code)
		{
			...
			throw new ArgumentException("Argument is not a local operator", nameof(code));
		}
		...
		public static bool TryApplyUpgradeTo(
			ref Item target, Item? upgrade, GameLocation? where,
			Farmer? who, bool probe, bool objectOnly, out bool isUpgrade, out string? notif
		)
		{
			isUpgrade = false;
			notif = null;

			if (upgrade is null || upgrade.IsRecipe)
				return false;

			if (!Assets.Data.TryGetValue(upgrade.QualifiedItemId, out var upgrades))
				return false;

			return TryApplyUpgradeTo(ref target, upgrades, where, who, probe, objectOnly, out isUpgrade, out notif);
		}

		public static bool TryApplyUpgradeTo(
			ref Item target, Dictionary<string, UpgradeEntry> upgrades, GameLocation? where, 
			Farmer? who, bool probe, bool objectOnly, out bool isUpgrade, out string? notif
		)
		{
			isUpgrade = false;
			...
			if (target is null || target.IsRecipe)
				return false;
			...
			if (entry.Condition is string cond && !GameStateQuery.CheckConditions(cond, where, who, target))
			{
